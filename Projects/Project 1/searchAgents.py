...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if pos in self.corners:
            if not pos in cornersVisited:
                cornersVisited.append(pos)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitWall:
                successorVisitedCorners = list(cornersVisited)
                nextPos = (nextx, nexty)
                if nextPos in self.corners:
                    if not nextPos in successorVisitedCorners:
                        successorVisitedCorners.append(nextPos)
                successor = ((nextPos, successorVisitedCorners), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    notVisited = set(corners) - cornersVisited
    if notVisited:
        return max(mazeDistance((x, y), corner, problem.startingGameState) for corner in notVisited)
    else: return 0
