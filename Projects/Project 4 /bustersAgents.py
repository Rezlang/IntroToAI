...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghostPositions = [(i.argMax(), i)
                          for i in livingGhostPositionDistributions]
        closestDist = float('inf')
        bestGhostPos = None

        ghostDists = [(self.distancer.getDistance(
            ghostPos[0], pacmanPosition), ghostPos[0]) for ghostPos in ghostPositions]
        for ghost in ghostDists:
            if ghost[0] < closestDist:
                closestDist = ghost[0]
                bestGhostPos = ghost[1]

        bestAction = None
        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action)
            newDist = self.distancer.getDistance(
                bestGhostPos, nextPos)
            if not bestAction or newDist < bestAction[1]:
                bestAction = (action, newDist)
        return bestAction[0]
