...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.allPositions:
            newDist[ghostPos] = self.beliefs[ghostPos] * \
                self.getObservationProb(observation, pacPos, ghostPos, jailPos)
        newDist.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist.keys():
                newDist[newPos] += oldProb * newPosDist[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.particles:
            prob = self.getObservationProb(
                observation, pacPos, ghostPos, jailPos)
            dist[ghostPos] += prob
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        particleCache = {}
        for i in range(self.numParticles):
            if self.particles[i] not in particleCache:
                newPosDist = self.getPositionDistribution(
                    gameState, self.particles[i])
                particleCache[self.particles[i]] = newPosDist
            self.particles[i] = particleCache[self.particles[i]].sample()
